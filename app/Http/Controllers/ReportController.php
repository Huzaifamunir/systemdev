...
class ReportController extends Controller{
    ...
    public function change_status_rejected($task_id){
        $edit_task=Task::find($task_id);
    ...
    public function screenshots(){
        ...
        $mainfloder=ScreenShots::all();
        $folderName=array();
        foreach($mainfloder as $folder)
        {
            array_push($folderName,$folder->folder);
        }
        $foldersarray = array_unique($folderName);
        return view('admin.screenshot',compact('foldersarray'));
    ...
    {
        $name=$id;
        $subfloder=ScreenShots::where('folder',$id)->get();
        $subfolderName=array();
        foreach($subfloder as $folder)
        {
            
            array_push($subfolderName,$folder->date);
        }
        $subfoldersarray = array_unique($subfolderName);
        return view('admin.subfolder',compact('subfoldersarray','name'));
    }
    
    public function getscreenshots(Request $request)
    {
        
        $screenshots=ScreenShots::where('date',$request->date)->where('folder',$request->folder)->get();
        $screenshotsarray=array();
        foreach($screenshots as $folder)
        {
            
            array_push($screenshotsarray,$folder->link);
        }
        // $subfoldersarray = array_unique($subfolderName);
        return view('admin.showss',compact('screenshotsarray'));
    }
    
    public function deletess($id)
    {
        \Storage::disk('s3')->deleteDirectory($id);
        $delete=ScreenShots::where('folder',$id)->delete();
        return back();
    }
